# Schema Events — AGENTOBS-Standard Observability

`llm-diff` adheres to the [AGENTOBS standard (RFC-0001)](https://www.getspanforge.com/standard)
and uses the [`agentobs`](https://pypi.org/project/agentobs/) SDK as a built-in,
declared dependency.  A structured, validated AGENTOBS event is emitted for every
significant pipeline operation — model call, cache lookup, diff, cost record,
judge evaluation, and `--fail-under` regression failure.  Events are
**collected in memory by default** and can also be exported to JSONL,
...
`agentobs` is a declared runtime dependency, so it is installed automatically:

```bash
pip install "llm-diff[semantic]"
```

No separate `agentobs` install is required.

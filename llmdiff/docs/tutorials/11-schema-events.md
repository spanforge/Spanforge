...
`llm-diff` solves this by adhering to the
[**AGENTOBS standard (RFC-0001)**](https://www.getspanforge.com/standard) —
the open event-schema standard for observability of agentic AI systems.  The
`agentobs` SDK is a declared runtime dependency of `llm-diff`, so no extra
install is required.  Every significant operation emits a structured,
timestamped, ULID-identified AGENTOBS event that conforms to well-defined
payload schemas.

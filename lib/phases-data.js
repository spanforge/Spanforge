export const phases = [
  ...
  },
  {
    ...
    summary: [
      ...
      'Includes tooling that benchmarks local LLM infrastructure and produces a model readiness score — so teams know exactly what they can run before committing to an architecture. CostGuard™ closes the cost visibility gap: every architecture decision carries a cost estimate before any resource is committed. Capability and cost — both known before a single line of production code is written.',
    ],
    gate: 'Nothing progresses to Build without: architecture documented, data strategy validated, security reviewed, team capacity confirmed, and CostGuard™ cost estimate produced with scenario comparison documented.',
  },
  {
    num: '03',
    id: 'build',
  ...
  },
  {
    ...
    tagline: 'SpanForge Platform. Production-ready.',
    summary: [
      'SpanForge is the Scale phase — the production observability and governance platform that connects to deployed AI agents via OpenTelemetry and watches what they actually do. Not just whether they are running — but whether they are behaving correctly.',
      'An agent that silently drifts and makes thousands of slightly wrong decisions is far more dangerous than one that crashes visibly. SpanForge makes the invisible visible — before regulators, users, or incident reports find the problem first.',
    ],
    gate: 'SpanForge active before the first production request. Behavioural baseline established. Drift monitoring configured. On-call owner named and playbooks tested.',
  },
]
...
export const trustDimensions = [
  ...
  { letter: 'R', word: 'Responsibility',    desc: 'A named human is accountable for every AI system. AI cannot be deployed without a designated owner who carries accountability for its behaviour in production. Responsibility extends to cost: CostGuard™ makes infrastructure cost visible at Design time, ensuring that accountable owners understand the financial implications of their architecture decisions before committing to them.' },
